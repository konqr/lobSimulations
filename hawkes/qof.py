...
def runACF(paths, resultsPath, sDate, eDate, ric):
    ...
    plt.scatter(statsmodels.tsa.stattools.acf(np.array(empRets).flatten(), nlags = 10000), label = "Empirical")
    plt.scatter(statsmodels.tsa.stattools.acf(np.array(simRets).flatten(), nlags = 10000), label = "Simulated")
    # plt.yscale("log")
    plt.legend()
