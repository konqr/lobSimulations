...
class ConditionalLeastSquaresLogLin():
    ...
    def fit(self):
        ...
        for i in self.dates:
            ...
            models = [SGDRegressor(penalty = None, max_iter=5000, verbose=2, learning_rate = "adaptive", eta0 = 1e-6).fit(Xs, Ys[:,i]) for i in range(Ys.shape[1])]
    ...
    def fitConditionalTimeOfDay(self):
        ...
        for i in self.dates:
            ...
            # params = model.coef_
            Ys = np.array(Ys)
            models = [SGDRegressor(penalty = None, fit_intercept=False, max_iter=5000, verbose=2, learning_rate = "adaptive", eta0 = 1e-6).fit(Xs, Ys[:,i]) for i in range(Ys.shape[1])]
    ...
    def fitConditionalTimeOfDayInSpread(self, spreadBeta = 0.41):
        ...
        for i in self.dates:
            ...
            # params2 = model.coef_
            model = SGDRegressor(penalty = None, fit_intercept=False, max_iter=5000, verbose=2, learning_rate = "adaptive", eta0 = 1e-6).fit(XsIS, Ys_inspreadBid)
            params2 = model.coef_

            Ys_inspreadAsk = [res_d[i][6] for i in range(0,len(res_d),2)]
            model = SGDRegressor(penalty = None, fit_intercept=False, max_iter=5000, verbose=2, learning_rate = "adaptive", eta0 = 1e-6).fit(XsIS, Ys_inspreadAsk)
            params3 = model.coef_
            ...
            models = [SGDRegressor(penalty = None, fit_intercept=False, max_iter=5000, verbose=2, learning_rate = "adaptive", eta0 = 1e-6).fit(Xs_oth, Ys_oth[:,i]) for i in range(Ys_oth.shape[1])]
