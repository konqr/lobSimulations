...
class MLE():
    ...
    def fit(self, tol=1e-6, max_iter=100000, elastic_net_ratio = 0):
        hawkes_learner = HawkesSumExpKern(decays = [1.7e3, 0.9*1.7e3, 0.99*1.7e3, 0.999*1.7e3], verbose = True, tol=tol, max_iter=max_iter, elastic_net_ratio =elastic_net_ratio)
        hawkes_learner.fit(self.data)
        baseline = hawkes_learner.baseline
